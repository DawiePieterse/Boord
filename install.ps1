...
"@
    ...
    Write-Host " setup_watchdog.bat  - checks every 5 minutes that the servers"
    Write-Host "                       on this PC are answering, and restarts"
    Write-Host "                       any that have stopped. Covers Boord,"
    Write-Host "                       Boord Owner, Boord Notes and Kudde."
    Write-Host " setup_heartbeat.bat - emails you if the server goes down for"
    Write-Host "                       more than an hour (needs a free"
    Write-Host "                       healthchecks.io account first). Pair it"
    Write-Host "                       with the watchdog: that one cannot tell"
    Write-Host "                       you when this PC itself is off."
    Write-Host " setup_update_check.bat - checks once a day whether a new"
    Write-Host "                       release is out and says so in Settings."
    Write-Host "                       It never installs anything by itself."
} catch {
    Write-Host ""
